...
console.log('Arbitrum:', balances.balances['arbitrum']);
...
| Campo | Descripcion |
|-------|-------------|
| `symbol` | Simbolo del token (USDC, ETH, etc.) |
...
| `usdValue` | Valor en USD del balance |
...
│                Pan API                      │
│                                             │
│  1. Solicita wallet a Privy                 │
│  2. Almacena mapeo userId -> walletId       │
│  3. Retorna direccion                       │
│                    │                        │
└────────────────────┼────────────────────────┘
                     │
...
│                  Privy                      │
│                                             │
│  1. Genera llave privada                    │
│  2. Deriva direccion                        │
│  3. Almacena llave encriptada               │
│  4. Retorna direccion (no la llave)         │
│                                             │
└────────────────────────────────────────────┘
```
...
async function getFormattedBalances(walletId) {
  const { balances, totalUsdValue } = await pan.wallet.getBalances(walletId);
