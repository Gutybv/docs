...
class WalletService {
  ...
  async getBalancesForDisplay(userId) {
    ...
    const { balances, totalUsdValue } = await this.pan.wallet.getBalances(
      user.panWalletId
    );
    ...
    for (const [chain, tokens] of Object.entries(balances)) {
      for (const token of tokens) {
        if (parseFloat(token.balanceFormatted) > 0) {
          formatted.push({
            chain: this.formatChainName(chain),
            chainId: chain,
            token: token.symbol,
...
function WalletCard({ userId }) {
  ...
  return (
    <div className="wallet-card">
      ...
      <h4>Balances</h4>
      {balances?.totalUsdValue > 0 ? (
        <>
          {Object.entries(balances.balances).map(([chain, tokens]) =>
            tokens.map(token => (
              <div key={`${chain}-${token.symbol}`} className="balance-row">
                <span>{chain}</span>
                <span>{token.balanceFormatted} {token.symbol}</span>
                <span>${token.usdValue.toFixed(2)}</span>
              </div>
            ))
          )}
          <div className="total">
            Total: ${balances.totalUsdValue.toFixed(2)}
          </div>
        </>
      ) : (
