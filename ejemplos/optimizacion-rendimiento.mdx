...
class YieldOptimizer {
  ...
  async findRebalanceOpportunities(
    ...
    for (const position of positions) {
      ...
      const betterYields = yields.yields.filter(y =>
...
async function calculateProjections(
  ...
  const bestYield = yields.yields
    .filter(y => y.asset === 'USDC')
    .sort((a, b) => b.apy - a.apy)[0];

  const apy = bestYield?.apy || 0.05; // Default 5%
...
function YieldOptimizer({ walletId }: { walletId: string }) {
  ...
  async function loadData() {
    ...
    for (const [chain, tokens] of Object.entries(balancesData.balances)) {
      const deposited = tokens.find((t: any) => t.protocol);
      if (deposited) {
        setCurrentPosition({
          chain,
          apy: deposited.apy || 0,
  ...
  return (
    <div className="space-y-6">
      ...
      {currentPosition && bestYield && bestYield.apy > currentPosition.apy && (
        <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg p-6 text-white">
          <h3 className="text-lg font-semibold mb-2">💡 Oportunidad de Optimizacion</h3>
          <p className="mb-4">
            Puedes ganar <strong>+{((bestYield.apy - currentPosition.apy) * 100).toFixed(2)}%</strong> mas
            moviendo a {bestYield.chain}
          </p>
          <button
            onClick={() => optimize(bestYield.chain)}
            disabled={optimizing}
