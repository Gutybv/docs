...
npm install express @odisea-labs/pan prisma @prisma/client dotenv zod
...
import { Pan, Wallet, Balances, Intent, PanError } from '@odisea-labs/pan';
...
class PanService {
  ...
  async getOrCreateWallet(userId: string, email?: string): Promise<Wallet> {
    ...
    try {
      ...
      if (error instanceof PanError && error.code === 'WALLET_LIMIT_EXCEEDED') {
