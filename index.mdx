...
def ejemplo_completo():
    ...
    for chain, tokens in balances['balances'].items():
        for token in tokens:
            print(f"  {chain}: {token['balanceFormatted']} {token['symbol']}")
...
flowchart LR
    ...
    subgraph PanAPI["Pan API"]
        ...
        subgraph Planning["Planificacion"]
            IP["Intent<br/>Planner"]
            BA["Balance<br/>Analyzer"]
            YS["Yield<br/>Service"]
        end

        subgraph Execution["Ejecucion"]
            EE["Execution<br/>Engine"]
            Bridge["Bridge<br/>Executor"]
            Deposit["Deposit<br/>Executor"]
            Withdraw["Withdraw<br/>Executor"]
        end
    ...
    style Chains fill:#f3e5f5,stroke:#7b1fa2
    style DeFi fill:#fff9c4,stroke:#f9a825
```
